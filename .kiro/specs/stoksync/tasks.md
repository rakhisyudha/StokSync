...
- [x] 4.8 Wire the authenticated composition root: add a login flow that obtains and stores a session via `SecureSyncSessionStore`, construct one `SyncSessionManager` from that session, provide it to `HttpSyncTransport` and `AuthenticatedHealthReachability`, construct a `SyncEngine` with `DriftSyncStatusStore`, and override `syncRuntimeProvider` in `main.dart` with the real `SyncRuntimeBinding.fromEngine(...)` so the existing sync stack (tasks 3.5-4.7) is actually reachable from the running app instead of only from tests. Production authenticated composition-root installation and atomic own-response sequence reconciliation are verified end-to-end against a live server.
- [x] 4.9 Build a deterministic two-device integration harness using independent local stores and a real Postgres database, exercising the wired composition root from 4.8 rather than fakes alone. Live validation against the migrated PostgreSQL-backed API proved two independent authenticated Drift replicas converge after product pull, offline movements, cursor-write interruption rollback and recovery, both push orders, tombstone propagation, and agreement of products, tombstones, movements, balances, and final cursor with the server snapshot.
