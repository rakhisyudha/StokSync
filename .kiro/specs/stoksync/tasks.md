...
- [x] 4.7 Implement access-token refresh-once behavior and blocked-sync state without wiping local data.
- [ ] 4.8 Wire the authenticated composition root: add a login flow that obtains and stores a session via `SecureSyncSessionStore`, construct one `SyncSessionManager` from that session, provide it to `HttpSyncTransport` and `AuthenticatedHealthReachability`, construct a `SyncEngine` with `DriftSyncStatusStore`, and override `syncRuntimeProvider` in `main.dart` with the real `SyncRuntimeBinding.fromEngine(...)` so the existing sync stack (tasks 3.5-4.7) is actually reachable from the running app instead of only from tests. Implementation verified working end-to-end against a live server; not yet marked complete.
- [ ] 4.9 Build a deterministic two-device integration harness using independent local stores and a real Postgres database, exercising the wired composition root from 4.8 rather than fakes alone. Blocked on a confirmed, reproducible defect: when a device pushes a new product/movement and the same sync response also returns that entity as a pull change (the normal case for a device's own first-time mutation), `DriftSyncResponseApplier` runs both `DriftSyncResponseReconciler` and `DriftSyncPageApplier` against the identical change. The reconciler writes the row first using Drift's default second-precision `dateTime()` columns; the page applier then re-applies the same-version change with the server's full microsecond-precision timestamp from the raw JSON payload, `_sameProduct`'s field equality check fails on `updatedAt`/`createdAt`, and `DriftRemoteChangeApplier._applyProductChange` throws `SyncProtocolException: contains conflicting product data for an existing version` (reproduced live in `app/test/integration/two_device_convergence_test.dart` against a fresh Postgres instance; deterministic, not environment-flaky). Fix before retrying 4.9: either have `DriftSyncResponseApplier`/`DriftSyncPageApplier` skip re-applying a pull-page change whose `seq` was already reconciled via that response's own push results, or make the same-version equality comparison timestamp-tolerant, or store `updatedAt`/`createdAt`/movement timestamp columns with microsecond precision so local round-trips match the server payload exactly.
